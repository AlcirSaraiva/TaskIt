...
public class Java_AES_Cipher {
    ...
    public static String prime(String tempIn) {
        String tempOut = "";
        for (int i = 0; i < tempIn.length(); i += 3) {
            tempOut += tempIn.charAt(i);
        }
        return tempOut;
    }

    public static String encryptSimple(String data) {
        String encPass[] = encrypt(prime("2gjdsvpke3zyhhchqlc5xf893dz5jqfoezcjhzjhwymnscxk"), prime("oagolq75v4iqrlzirubocwqr6bsj83z9j2f10bhmj2kf748f"), data).split(":");
        if (encPass.length > 0) {
            return encPass[0];
        } else {
            return "";
